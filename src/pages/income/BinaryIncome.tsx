...
const BinaryIncome = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await api.get<IncomeResponse>("/wallet/income/binary", { params: { skip, take: TAKE } });
      setTransactions(response.data?.transactions || []); setTotal(response.data?.total || "0"); setCount(response.data?.count || 0);
    } catch (error: any) { toast({ title: t("common.error"), description: error?.response?.data?.message || "Failed to fetch binary income", variant: "destructive" }); setTransactions([]); setTotal("0"); setCount(0); }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-foreground">{t("income.binaryIncome")}</h1>
      <Card className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border-green-500/20">
        <CardContent className="p-6">
          <div className="flex items-center gap-4">
            ...
            <div><p className="text-sm text-muted-foreground">{t("income.totalBinaryEarned")}</p><p className="text-3xl font-bold text-foreground">${parseFloat(total).toLocaleString()}</p></div>
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <CardTitle>{t("income.binaryTransactions")}</CardTitle>
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-4">{[...Array(5)].map((_, i) => (<Skeleton key={i} className="h-12 w-full" />))}</div>
            ...
            <div className="text-center py-12"><TrendingUp className="h-12 w-12 mx-auto text-muted-foreground/50 mb-4" /><p className="text-muted-foreground">{t("income.noBinaryRecords")}</p><p className="text-sm text-muted-foreground/70 mt-1">{t("income.binaryAppearHere")}</p></div>
          ) : (
            <>
