...
const ReferralIncome = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await api.get<IncomeResponse>("/wallet/income/referral", { params: { skip, take: TAKE } });
      setTransactions(response.data?.transactions || []); setTotal(response.data?.total || "0"); setCount(response.data?.count || 0);
    } catch (error: any) { toast({ title: t("common.error"), description: error?.response?.data?.message || "Failed to fetch referral income", variant: "destructive" }); setTransactions([]); setTotal("0"); setCount(0); }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-foreground">{t("income.referralIncome")}</h1>
      <Card className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 border-green-500/20">
        <CardContent className="p-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center"><TrendingUp className="h-6 w-6 text-green-500" /></div>
            <div><p className="text-sm text-muted-foreground">{t("income.totalReferralEarned")}</p><p className="text-3xl font-bold text-foreground">${parseFloat(total).toLocaleString()}</p></div>
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <CardTitle>{t("income.referralTransactions")}</CardTitle>
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-4">{[...Array(5)].map((_, i) => (<Skeleton key={i} className="h-12 w-full" />))}</div>
            ...
            <div className="text-center py-12"><TrendingUp className="h-12 w-12 mx-auto text-muted-foreground/50 mb-4" /><p className="text-muted-foreground">{t("income.noReferralRecords")}</p><p className="text-sm text-muted-foreground/70 mt-1">{t("income.referralAppearHere")}</p></div>
          ) : (
            <>
