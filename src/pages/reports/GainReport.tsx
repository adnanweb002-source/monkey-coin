...
import {
  TrendingUp, Calendar, RefreshCw, RotateCcw, ChevronLeft, ChevronRight, ArrowDownLeft, ArrowUpRight,
} from "lucide-react";
...
const GainReport = () => {
  ...
  const fetchGainReport = async () => {
    if (!fromDate || !toDate) {
      toast({ title: t("common.error"), description: t("reports.selectDateRange"), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: t("common.error"), description: error.response?.data?.message || t("reports.gainReport"), variant: "destructive" });
      setData(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const setDatePreset = (preset: "today" | "week" | "month") => {
    ...
    switch (preset) {
      case "today": setFromDate(format(today, "yyyy-MM-dd")); setToDate(format(today, "yyyy-MM-dd")); break;
      case "week": setFromDate(format(subDays(today, 7), "yyyy-MM-dd")); setToDate(format(today, "yyyy-MM-dd")); break;
      case "month": setFromDate(format(startOfMonth(today), "yyyy-MM-dd")); setToDate(format(today, "yyyy-MM-dd")); break;
    }
  };
  ...
  useEffect(() => { setDatePreset("month"); fetchGainReport(); }, [type, self]);

  const currentPage = Math.floor(skip / TAKE) + 1;
  const totalPages = Math.ceil(count / TAKE);

  useEffect(() => { fetchGainReport(); }, [skip]);

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-foreground">{getReportTitle()}</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setDatePreset("today")}>{t("reports.today")}</Button>
            <Button variant="outline" size="sm" onClick={() => setDatePreset("week")}>{t("reports.thisWeek")}</Button>
            <Button variant="outline" size="sm" onClick={() => setDatePreset("month")}>{t("reports.thisMonth")}</Button>
          </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-foreground">{data ? formatCurrency(data.total) : formatCurrency("0")}</p>
                  <p className="text-xs text-muted-foreground mt-1">{fromDate} to {toDate}</p>
                </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <Button variant="outline" size="sm" onClick={fetchGainReport} disabled={isLoading}>
            ...
            <CardContent>
              {data?.transactions.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">{t("reports.noRecordsYet")}</p>
                  <p className="text-sm text-muted-foreground/70 mt-1">{t("reports.transactionsAppear")}</p>
                </div>
              ) : (
                <>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right">{t("income.balanceAfter")}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                <Badge className="bg-green-500/20 text-green-500 hover:bg-green-500/30">
                                  {type === "PACKAGE_PURCHASE" ? <ArrowUpRight className="h-3 w-3 mr-1" /> : <ArrowDownLeft className="h-3 w-3 mr-1" />}
                                  {type === "PACKAGE_PURCHASE" ? t("income.debit") : t("income.credit")}
                                </Badge>
                                <span className="font-medium text-green-500">+${parseFloat(tx.amount).toLocaleString()}</span>
                              </div>
                            </TableCell>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                      <p className="text-sm text-muted-foreground">
                        {t("common.showing")} {skip + 1}-{Math.min(skip + TAKE, count)} {t("common.of")} {count} {t("common.transactions")}
                      </p>
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => setSkip(Math.max(0, skip - TAKE))} disabled={skip === 0}>
                        ...
                        <Button variant="outline" size="sm" onClick={() => setSkip(skip + TAKE)} disabled={skip + TAKE >= count}>
        ...
        <Card>
          <CardContent className="py-12">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-lg font-medium">{t("reports.selectDateRange")}</p>
